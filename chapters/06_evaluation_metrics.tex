...
It is interesting to see that each method by itself doesn't remove a great amount of words. The method that removed more words by itself was {\color{red} whatever }, at an order of x\%. On the other hand, the method y by itself removed only y\% of words. All methods combined where able to reduce the \ac{VSM} size in about z\%.
