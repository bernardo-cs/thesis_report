...
The WEBSOM was not based on keywords and boolean expressions, instead, words with the same meaning are encoded in a word category map (Fig~\ref{chp3:img3}), where placement and frequency in documents is taken into account. This way it is possible to remove words with similar meaning, greatly reducing the \ac{VSM} size and thus making it possible to train the document map in a scalable way.
...
By including social features into clustering algorithms, it might be possible to discover new clusters of topics that have greater significance to the users of social networks.
